...
const ProjectExperience = ({ projectName, projectDetails, media, footerLink }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const buttons = document.querySelectorAll('.previous-project-button, .next-project-button');
      const stopPosition = 100; // distance from the bottom of the page in pixels
      const documentHeight = document.documentElement.scrollHeight;
      const windowHeight = window.innerHeight;
      const scrollPosition = window.scrollY || window.pageYOffset;
      const bottomReached = (documentHeight - windowHeight - scrollPosition) <= stopPosition;
  ...
  const currentIndex = projects.findIndex(project => project.projectName === projectName);
        ...
        <div className="project-content">
          ...
          <div className="media-container center-content">
            {media.map((item, index) => (
                <Fade key={index} left={index % 2 === 0} right={index % 2 !== 0}> {/* Apply Fade effect */}
                {item.type === 'text' ? (
                  <p className="project-text" style={{ width: item.width, marginBottom: '20px' }}>{item.src}</p>
                ) : item.type === 'image' ? (
                  <img src={item.src} alt={item.alt} className="project-image" style={{ width: item.width, height: item.height, marginBottom: '20px' }} />
                ) : item.type === 'video' ? (
                  <video controls className="project-video" style={{ width: item.width, height: item.height, marginBottom: '20px' }}>
          ...
          <button className="previous-project-button" onClick={() => navigate(previousProject.path)}>Previous Project</button>
          <button className="next-project-button" onClick={() => navigate(nextProject.path)}>Next Project</button>
        </div>
